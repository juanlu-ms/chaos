...
* **Run History Persistence:**
    * **FileRunHistory:** File-system-backed implementation of IRunHistory. Stores each run as `<dir>/<id>.json`. Maintains an in-memory summary cache for fast listing (sorted by start time descending). Retention capped at N most-recent runs; oldest pruned on each save. Thread-safe via internal mutex.
