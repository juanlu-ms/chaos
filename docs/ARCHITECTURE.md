...
* **Orchestration Layer:**
    ...
    * **CompositeRunObserver:** Multiplexer that fans out observer events to multiple IRunObserver implementations. Wraps each forward in try/catch with spdlog warn-level logging for exception isolation. Used to compose the existing observer (WebRunObserver / CliRunObserver) with RunRecorder without changing the single-observer signature of RunOrchestrator.
    * **RunRecorder:** IRunObserver implementation that accumulates time-series samples (CPU, memory, network, latency), phase-zone boundaries, and log tails during a run. Provides a finalize() method that returns a complete RunRecord for persistence. All mutable state is mutex-guarded for thread safety.
    * **RunResult (core module):** Standalone header (`core/RunResult.hpp`) containing the RunResult struct with pass/fail, per-expectation results, manifest name, target ID, duration, and ISO-8601 start timestamp. Reused by both the legacy serializer (`core::runResultToJson`) and the history module.
...
* **Run History Persistence:**
    * **FileRunHistory:** File-system-backed implementation of IRunHistory. Stores each run as `<dir>/<id>.json`. Maintains an in-memory summary cache for fast listing (sorted by start time descending). Retention capped at N most-recent runs; oldest pruned on each save. Thread-safe via internal shared_mutex (concurrent readers, exclusive writers).
    * **HistoryJson:** Free functions (`runRecordToJson`, `runRecordFromJson`, etc.) that serialize/deserialize RunRecord and RunSummary to/from JSON. Reuses `core::runResultToJson` for the embedded RunResult sub-object, keeping history in sync with future RunResult changes.
    * **IRunHistory:** Facade interface (save, list, get, remove, clear) consumed by both the Server (REST routes) and CliParser (history subcommand). Defaulted-to-nullptr injection means history is optional at both entry points.
