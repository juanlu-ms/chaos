...
* **Orchestration Layer:**
    ...
    * **ChaosRunner (core module):** Provides three operations: `buildPerturbations` (delegates to `PerturbationFactory`), `finalize` (delegates to `ValidationEngine` with optional continuous-failure tracking), and `parseManifest` (delegates to `ManifestParser`). Both `CliParser` and `Server` use `RunOrchestrator` which delegates to `ChaosRunner`.
