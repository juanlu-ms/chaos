...
export default function ChartBase({ data, dataKey, unit, title, formatter, themeKey, height = 160, domain = [0, 'auto'], children }) {
  const { accent, grid, text, surface } = useChartTheme();
    ...
    <div className="chart-card">
      ...
      <ResponsiveContainer width="100%" height={height}>
        <AreaChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
          ...
          <Tooltip contentStyle={{ background: surface, border: `1px solid ${grid}`, borderRadius: 6, fontSize: 11 }} labelFormatter={(v) => `t = ${Number(v).toFixed(1)}s`} formatter={(v) => yFormat(v)} />
          {children}
