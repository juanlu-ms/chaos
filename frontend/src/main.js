...
function createLiveChart(containerId, unit, color) {
  ...
  chart.setOption({
    ...
    xAxis: {
      ...
      nameTextStyle: { color: '#9e8f7e' },
      axisLabel: { color: '#9e8f7e' },
      splitLine: { lineStyle: { color: '#3d322844' } }
    },
    yAxis: {
...
function createResultsChart(containerId, unit, color) {
  const dom = document.getElementById(containerId);
  if (!dom) return null;
  const chart = echarts.init(dom);
  chart.setOption({
...
function pushChartData(chart, x, y) {
  if (!chart) return;
...
function buildZoneMarkers(dataPoints) {
  if (!dataPoints || dataPoints.length === 0) return [];
  const successC = getCSS('--success-muted') || 'rgba(76,175,80,0.1)';
  const errorC = getCSS('--error-muted') || 'rgba(239,83,80,0.1)';
  let chaosStart = null, recoveryStart = null;
  ...
  if (chaosStart !== null) {
    zones.push([
      { xAxis: dataPoints[0].x, itemStyle: { color: successC } },
      { xAxis: chaosStart }
    ]);
    if (recoveryStart !== null) {
      zones.push([
        { xAxis: chaosStart, itemStyle: { color: errorC } },
        { xAxis: recoveryStart }
      ]);
      zones.push([
        { xAxis: recoveryStart, itemStyle: { color: successC } },
        { xAxis: dataPoints[dataPoints.length - 1].x }
      ]);
    }
  }
  return zones;
...
function setResultsData(chart, dataPoints) {
  if (!chart) return;
  const data = dataPoints.map(d => [d.x, d.y]);
  const zones = buildZoneMarkers(dataPoints);
  chart.setOption({ series: [{ data, markArea: { silent: true, data: zones } }] });
}

function disposeCharts() {
  Object.values(liveCharts).forEach(c => { if (c) c.dispose(); });
  Object.values(resultsCharts).forEach(c => { if (c) c.dispose(); });
  liveCharts = {};
  resultsCharts = {};
}
...
function initCharts() {
  ...
  liveCharts.cpu = createLiveChart('chart-cpu-live', '%', colors.accent);
  liveCharts.mem = createLiveChart('chart-mem-live', 'MB', colors.info);
  liveCharts.net = createLiveChart('chart-net-live', 'B/s', colors.success);
}
...
function initTimelineCharts() {
  ...
  resultsCharts.cpu = createResultsChart('chart-cpu-results', '%', colors.accent);
  resultsCharts.mem = createResultsChart('chart-mem-results', 'MB', colors.info);
  resultsCharts.net = createResultsChart('chart-net-results', 'B/s', colors.success);
}
...
async function runScenario() {
  ...
  showStep(2);
  initCharts();
  Object.values(liveCharts).forEach(c => c?.resize());
  ...
  try {
    ...
    eventSource.addEventListener('complete', e => {
      try {
        ...
        showStep(3);
        initTimelineCharts();
        Object.values(resultsCharts).forEach(c => c?.resize());
    ...
    eventSource.addEventListener('error', e => {
      ...
      showResultsSummary({ passed: false, results: [{ type: 'error', passed: false, message: errorMsg }], logs: accumulatedLogs });
      showStep(3);
      initTimelineCharts();
      Object.values(resultsCharts).forEach(c => c?.resize());
      cleanupRun();
    ...
    showResultsSummary({ passed: false, results: [{ type: 'error', passed: false, message: err.message }], logs: accumulatedLogs });
    showStep(3);
    initTimelineCharts();
    Object.values(resultsCharts).forEach(c => c?.resize());
    cleanupRun();
...
function showResultsSummary(data) {
  ...
  const targetId = lastManifest ? lastManifest.target.id : '--';
  document.getElementById('r-target').textContent = targetId.length > 15 ? targetId.substring(0, 12) + '...' : targetId;
  const perts = lastManifest ? lastManifest.perturbations : [];
  document.getElementById('r-perturbations').textContent = perts.length > 0
    ? `${perts.length} (${perts.map(p => p.type).join(', ')})`
    : 'None';
...
function updateChartTheme() {
  ...
  const info = getCSS('--info');
  const success = getCSS('--success');
  const textDim = getCSS('--text-dim');
  const borderC = getCSS('--border');

  [liveCharts, resultsCharts].forEach(chartMap => {
    Object.entries(chartMap).forEach(([key, chart]) => {
      if (!chart) return;
      const color = key === 'cpu' ? accent : key === 'mem' ? info : success;
      chart.setOption({
        xAxis: {
          nameTextStyle: { color: textDim },
          axisLabel: { color: textDim },
          splitLine: { lineStyle: { color: borderC + '44' } }
        },
        yAxis: {
          nameTextStyle: { color: textDim },
          axisLabel: { color: textDim },
...
document.getElementById('perturbation-list').addEventListener('input', e => {
  ...
  if (inp) perturbations[parseInt(inp.dataset.idx)].params[inp.dataset.key] = inp.value;
});
