...
RunRecorder::RunRecorder(const manifests::ChaosManifest& manifest)
    : manifest_(manifest), start_steady_(steady_clock::now()), start_system_(system_clock::now()) {
    auto ms = duration_cast<milliseconds>(start_system_.time_since_epoch()).count();
    summary_.id = "run-" + std::to_string(ms);
...
RunRecord RunRecorder::finalize(const core::RunResult& runResult, std::string status, std::string error) {
    ...
    return RunRecord{summary_, manifest_, samples_, logs_};
}
