 ...
 *   src=0x01010101 (1.1.1.1), dst=0x02020202 (2.2.2.2), tcpLen=20
...
TEST(PacketFloodInternalTest, PseudoHeaderChecksumKnownVector) {
    ...
    uint32_t sum = pseudoHeaderChecksum(srcIp, dstIp, tcpLen);
...
TEST(PacketFloodInternalTest, SegmentChecksumKnownVector) {
    ...
    uint32_t pseudoSum = 0x000F012F;

    uint16_t checksum = segmentChecksum(tcpSegment, pseudoSum);
